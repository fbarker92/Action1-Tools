...
### Nextcloud install\update

function main() {
  # internal parameters
    display_name='Nextcloud' # for messages only
    log -m "$(printf 'start deploying "%s"' "$display_name")" -n "INFO"

    proc_names=("Nextcloud$") # array, f.e. ("process1" "process2")
    default_app_folder="Nextcloud.app"  # for install mode
